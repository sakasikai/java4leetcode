...
//        int[] in = {4,13,1,1,14,15,1,3,13,1,3,5,2,8,12};
    ...
    public boolean makesquare$3(int[] matchsticks) {
        ...
        anEdge = stks.stream().reduce(Integer::sum).orElse(-1);
        if (anEdge % 4 != 0) return false;
        anEdge /= 4;
    ...
    public boolean doMatch(int curLen, int round, int pivot){
        ...
        for (int i=pivot; i<stks.size(); i++) {
            ...
            // slen fails
            if (stks.get(i) == anEdge - curLen) return false; // TODO slen 失败了，后续所有round都失败，直接停止

            int j = i + 1;
            while (j < stks.size() && (state >> i & 1) == 0 && stks.get(j).equals(stks.get(i))) {
